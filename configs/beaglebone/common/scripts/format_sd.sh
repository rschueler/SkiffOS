...
sudo dd if=/dev/zero of=$BEAGLEBONE_SD bs=8k count=13 oflag=dsync
...
sudo parted -a optimal $BEAGLEBONE_SD -- mkpart primary ext4 10MiB "100%"
...
mkfs.ext4 -F -L "persist" ${BEAGLEBONE_SD_SFX}1

sync && sync

echo "Flashing MLO..."
dd iflag=dsync oflag=dsync if=$bootmlo of=$BEAGLEBONE_SD bs=128k count=2 seek=1 ${SD_FUSE_DD_ARGS}

echo "Flashing u-boot..."
dd iflag=dsync oflag=dsync if=$ubootimg of=$BEAGLEBONE_SD seek=1 bs=384k ${SD_FUSE_DD_ARGS}
